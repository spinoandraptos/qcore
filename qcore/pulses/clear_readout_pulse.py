...
class ClearReadoutPulse(ReadoutPulse):
    ...
    def sample(self):
        """ """
        def cosine_edge_segment(amplitude, duration):
            duration = int(duration)
            if duration <= 0:
                return np.array([])
            t_vals = np.linspace(0, 1, duration, endpoint=False)
            window = np.sin(np.pi * t_vals / 2) ** 2
            return Pulse.BASE_AMP * amplitude * self.total_I_amp * window
        def constant_segment(amplitude, duration):
            duration = int(duration)
            return np.full(int(duration), (Pulse.BASE_AMP * amplitude* self.total_I_amp))
        # Construct segments
        ringup1 = cosine_edge_segment(self.ringup1_amp, self.ringup1_time)
        ringdown1 = cosine_edge_segment(self.ringdown1_amp, self.ringdown1_time)
        drive = constant_segment(self.drive_amp, self.drive_time)
        ringdown2 = cosine_edge_segment(self.ringdown2_amp, self.ringdown2_time)
        ringup2 = cosine_edge_segment(self.ringup2_amp, self.ringup2_time)
        waveform = np.concatenate([ringup1, ringdown1, drive, ringdown2, ringup2])
        i_samples = np.real(waveform)
        pad = np.zeros(self.pad) if self.pad else []
        i_wave = np.concatenate((i_samples, pad))
